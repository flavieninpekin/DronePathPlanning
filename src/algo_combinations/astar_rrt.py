...
def _sample_near_astar_point(astar_points: List[Tuple[float, ...]], grid: np.ndarray, ratio: int) -> np.ndarray:
    if not astar_points:
        return _sample_uniform(grid)
    max_attempts = 50
    dims = grid.ndim
    for _ in range(max_attempts):
        base = np.array(random.choice(astar_points))
...
def _sample_rrt_point(grid: np.ndarray, astar_points: List[Tuple[float, ...]], ratio: int, bias_prob: float) -> np.ndarray:
    if random.random() < bias_prob and astar_points:
        return _sample_near_astar_point(astar_points, grid, ratio)
    return _sample_uniform(grid)
...
def _rrt_biased(
    ...
    for _ in range(max_iter):
        rand_point = _sample_rrt_point(grid, astar_points, ratio, bias_prob)
...
def run_astar_rrt_pipeline(
    ...
    if density is not None:
        ...
        # density parameter is provided for metadata or validation only
    ...
    astar_path = Astar.astar(downsampled, start_ds, goal_ds)
    if not astar_path:
        return None

    astar_enlarged = _scale_path_to_original(astar_path, ratio)
    rrt_path = _rrt_biased(
        grid=grid,
        start=start,
        goal=goal,
        astar_points=astar_enlarged,
        ratio=ratio,
        bias_prob=bias_prob,
        step_size=step_size,
