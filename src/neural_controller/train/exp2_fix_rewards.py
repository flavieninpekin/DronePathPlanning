"""Exp2: Single drone + fixed speed reward direction + fixed stuck counter.

Speed reward fix: reward movement toward the CURRENT target waypoint,
  not along the path segment direction.
Stuck counter fix: increment when drone fails to make progress toward its waypoint.
"""
import torch
...
def _patched_step(self, actions):
    ...
    # ===== FIXED: stuck counter now increments =====
    for i in range(self.num_drones):
        if not self.drone_active[i]:
            continue
    ...
    for i, agent in enumerate(self.agents):
        ...
        # FIXED: speed reward toward CURRENT waypoint, not along path segment
        ...
        # FIXED: stuck detection now actually triggers
...
# =============================================================================
# Training (same as exp1 but with patched env)
# =============================================================================
def setup_logger():
    ...
    log_path = os.path.join(log_dir, f"exp2_fix_rewards_{ts}.log")
    logger = logging.getLogger("exp2_fix_rewards")
...
def main():
    ...
    num_episodes = int(os.environ.get("EXP_NUM_EPISODES", "10000"))
    ...
    runner = ParallelEnvRunner(base_config, num_envs=num_envs, single_map=single_map)
    agent = MAPPO(
    ...
    try:
        writer = SummaryWriter("runs/exp2_fix_rewards")
        model_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../models"))
        os.makedirs(model_dir, exist_ok=True)
        model_path = os.path.join(model_dir, "exp2_actor_latest.pth")
        best_a = os.path.join(model_dir, "exp2_actor_best.pth")
        best_c = os.path.join(model_dir, "exp2_critic_best.pth")

        best_score = -float("inf")

        logger.info("=" * 60)
        logger.info("EXP2 — Single Drone + Fixed Rewards")
        logger.info("  Fix 1: speed reward toward CURRENT waypoint (not path segment)")
        logger.info("  Fix 2: stuck counter now increments on no-progress")
