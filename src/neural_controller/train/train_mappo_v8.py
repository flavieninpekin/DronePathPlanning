...
def setup_logger():
    ...
    log_path = os.path.join(log_dir, f"train_mappo_v8_{ts}.log")
    logger = logging.getLogger("mappo_train_v8")
...
def main():
    ...
    # 当前 4 * 2048 = 8192，目标稀疏奖励任务建议扩大到 16384 或更高
    steps_per_env = 4096
    eval_episodes = 10
    ...
    try:
        writer = SummaryWriter("runs/mappo_v8")
        model_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../models"))
        os.makedirs(model_dir, exist_ok=True)
        model_path = os.path.join(model_dir, "actor_latest_v8.pth")
        best_a = os.path.join(model_dir, "actor_best_v8.pth")
        best_c = os.path.join(model_dir, "critic_best_v8.pth")
        ...
        logger.info("=" * 60)
        logger.info("MAPPO v8 — Metropolis + V2 Rewards")
        ...
        logger.info("  drones=6 grace=30 death=-80 stuck_max=100")
