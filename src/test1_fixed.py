...
def generate_scenario(num_tasks, num_drones, density, max_attempts=100):
    ...
    for attempt in range(max_attempts):
        ...
        if len(reachable) >= num_tasks + num_drones + 5:  # 留一些余量
...
def run_experiment_5():
    ...
    print("\n实验5完成。")
    print("  注意：此实验使用模拟数据演示收敛过程。")
    print("  如需真实数据，请修改拍卖算法返回每轮迭代的中间状态。")
...
def run_experiment_6():
    ...
    if variances and not all(np.isnan(v) for v in variances):
        ax1.set_ylim(0, max(v for v in variances if not np.isnan(v)) * 1.2)
    ...
    if ratios and not all(np.isnan(r) for r in ratios):
        ax2.set_ylim(0, max(r for r in ratios if not np.isnan(r)) * 1.2)
